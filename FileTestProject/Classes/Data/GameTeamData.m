...
-(instancetype)initWithTeamData:(TeamData *)teamData guildId:(NSString *)guildId
{
  if (self = [self init]) {
    ...
    _npcList = npcList;
    self.currentCityId = teamData.startCity;
  }
  return self;
}
...
- (void)setCurrentCityId:(NSString *)currentCityId
{
  ...
  CityData *cityData = [[DataManager sharedDataManager].getCityDic getCityById:currentCityId];
  _pos.x = cityData.longitude;
  _pos.y = cityData.latitude;
}
...
-(NSString *)relationToGuild:(NSString *)guildId
{
  ...
  NSAssert(guildData != nil, @"guild cannot be nil");
  if ([_belongToGuildId isEqualToString:guildId]) {
    if ([_leaderId isEqualToString:guildData.leaderId]) {
      return @"mine";
    } else {
      return @"secondary";
    }
