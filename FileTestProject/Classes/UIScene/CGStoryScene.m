      ...
      while (_currentIndex < _storyList.count && flag) {
        ...
        switch (storyData.command) {
          ...
          case StoryCommandTypeShowDialog:
          {
            ...
            NSString *portraitId = nil;
            if ([storyData.parameter1 isEqualToString:@"npc"] || [storyData.parameter1 isEqualToString:@"city"]) {
              portraitId = [NSString stringWithFormat:@"%@_portrait_%@.png",storyData.parameter1, storyData.parameter2];
            }
            [_dialogPanel setDialogWithPhotoNo:portraitId npcName:npcName text:getStoryText(storyData.parameter3)];
            _inDialog = YES;
            flag = NO;
